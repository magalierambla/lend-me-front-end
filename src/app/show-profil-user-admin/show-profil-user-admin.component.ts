...
import {UserModel, ProjectModel, templteProjectModel, StatutProjectModel,
        CategorieProjectModel, PorteProjectModel, ResponseConnectionUserModel} from '../interfaces/models';
...
export class ShowProfilUserAdminComponent implements OnInit {
  ...
  getInfosUser(tokenUser){
       ...
       this.apiService.getDataUser(this.ObjetResponseConnection, tokenUser).subscribe((dataUser: UserModel) => {
  ...
  getListAllProjects(tokenUser) {
    ...
    this.apiService.listMyProjectByUserByManager( this.ObjetResponseConnection, tokenUser).subscribe((dataMyProjects: Array<ProjectModel>) => {
    ...
    // tslint:disable-next-line:max-line-length
    this.apiService.listContribProjectByUserByManager( this.ObjetResponseConnection, tokenUser).subscribe((dataContribProject: Array<ProjectModel>) => {
  ...
  formaterListProject() {
    ...
    for (let index = 0; index < this.listCreatedProjects.length; index++) {
      ...
      // tslint:disable-next-line:max-line-length
     // this.listtemplateCreatedProjects[index].project._user.date_naissance = this.datePipe.transform(this.listtemplateCreatedProjects[index].project._user.date_naissance, 'dd-MM-yyyy');
